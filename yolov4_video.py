...
def detect(frm, net, ln):
    # (h, w) = frm.shape[:2]
    blob = cv2.dnn.blobFromImage(frm, 1 / 255, (416, 416), swapRB=True, crop=False)
    net.setInput(blob)
...
while cv2.waitKey(1) < 1:
    ...
    # width = 960
    # height = 576
    frame = cv2.resize(frame,dsize=None,dst=None,fx=0.6,fy=0.6,interpolation=None)

    width = frame.shape[1]
    height = frame.shape[0]

    frame = cv2.line(frame,(0,50),(width,50),(255,255,0),5)
    frame = cv2.line(frame,(0,height-50),(width-0,height-50),(255,255,0),5)
